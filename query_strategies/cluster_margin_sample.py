...
class ClusterMarginSampling(Strategy):
    ...
    def query(self, k, index, param2,param3):
        unlabeled_idxs, unlabeled_data = self.dataset.get_unlabeled_data(index = index)
        ...
        n = min(k*5,len(unlabeled_idxs))
        index = self.margin_data(n, index)
